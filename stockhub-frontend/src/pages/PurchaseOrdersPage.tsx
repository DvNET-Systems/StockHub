...
const schema = z.object({
  supplierId: z.coerce.number().min(1, 'Select a supplier'),
...
export function PurchaseOrdersPage() {
  ...
  // When a product is selected, auto-fill unit cost from product's cost price
  const handleProductChange = (index: number, productId: number) => {
    const product = products.find((p) => p.id === productId)
    if (product) setValue(`items.${index}.unitCost`, product.costPrice)
  }
  ...
  const completeMut = useMutation({
    mutationFn: purchaseOrdersApi.complete,
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['purchase-orders'] })
      qc.invalidateQueries({ queryKey: ['products'] })
      qc.invalidateQueries({ queryKey: ['stock'] })
      toast.success('Order completed! Stock added.')
    },
    onError: (e: Error) => toast.error(e.message),
  })
  ...
  const openCreate = () => {
    reset({ items: [{ productId: 0, quantity: 1, unitCost: 0 }] })
    setDialogOpen(true)
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-[var(--color-text)]">Purchase Orders</h1>
          <p className="text-sm text-[var(--color-text-muted)]">{orders.length} orders</p>
        </div>
        <Button icon={<Plus size={16} />} onClick={openCreate}>New Order</Button>
      </div>
      ...
      <div className="flex gap-2 flex-wrap text-xs text-[var(--color-text-muted)] items-center">
        ...
        <span className="text-[var(--color-green)]">Completed (stock IN)</span><span>·</span>
      ...
      {/* Create Dialog */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title="New Purchase Order" width="max-w-2xl">
        <form onSubmit={handleSubmit((d) => createMut.mutate(d))} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Select
              ...
              {...register('supplierId')}
            />
            <Input label="Notes (optional)" placeholder="Monthly restock…" {...register('notes')} />
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <div>
                ...
                <span className="text-xs text-[var(--color-text-dim)] ml-2">— unit cost auto-fills from product cost price</span>
              </div>
              <button
                type="button"
            ...
            {/* Header row */}
            <div className="grid grid-cols-[1fr_90px_110px_28px] gap-2 px-0">
              <span className="text-xs text-[var(--color-text-dim)]">Product</span>
              <span className="text-xs text-[var(--color-text-dim)]">Qty</span>
              <span className="text-xs text-[var(--color-text-dim)]">Unit Cost</span>
              <span />
            </div>
            ...
            {fields.map((field, i) => (
              <div key={field.id} className="grid grid-cols-[1fr_90px_110px_28px] gap-2 items-start">
                ...
                <Input
                  ...
                  type="number"
                  step="0.01"
                  error={errors.items?.[i]?.unitCost?.message}
                  {...register(`items.${i}.unitCost`)}
                />
                <button
                  type="button"
                  onClick={() => remove(i)}
...
function OrderRow({
  ...
  // Normalize status — handles both integer (0,1,2,3) and string ('Draft' etc.)
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <p className="text-xs text-[var(--color-text-dim)]">
              {order.supplierName} · {format(parseISO(order.orderDate), 'MMM d, yyyy')}
            </p>
          </div>
          ...
          <table className="w-full text-xs mt-3 mb-4">
            <thead>
              <tr className="text-[var(--color-text-muted)]">
                ...
                <td className="py-1 text-right">Unit Cost</td>
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {order.items.map((i) => (
                <tr key={i.id}>
                  ...
                  <td className="py-2 text-right font-mono">${i.unitCost.toFixed(2)}</td>
                  <td className="py-2 text-right font-mono font-medium text-[var(--color-text)]">
